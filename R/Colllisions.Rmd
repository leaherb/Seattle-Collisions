...
# Rename Columns for easier reading of code and plots
df <- setnames(df,
...
df$Year <- lubridate::year(as.Date(df$INCDATE))
df$Month <- lubridate::month(as.Date(df$INCDATE))

# Delete records not between 2004-2018 (our report range)
df <- subset(df, Year < 2019 & Year > 2003)

df$Year <- as.factor(df$Year)
df$Month <- as.factor(df$Month)

# Extract Time and Hour(only) from INCDTTM
...
# There is 1 record where SEVERITYCODE = ''. Delete it.
df <- subset(df, SEVERITYCODE != '')


# Group collision types into fewer categories.
df$SDOTtype <- factor(with(df,
     ...
     ifelse(SDOT_COLCODE >= 40 & SDOT_COLCODE <= 51, 'Struck Object',
...
# Standardize DUI values ... assume N, Y, '' (unknown)
...
# Shorten SEVERITYDESC by removing ' Collision'
