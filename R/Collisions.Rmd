...
```{r echo=FALSE, warning=FALSE, message=FALSE, Load_data}
  # from local file (downloaded manually)
df = read.csv('Collisions.csv', na.strings=c(""," "), header=TRUE)
  # from data-seattlecitygis (latest data available)
#df = read.csv('http://data-seattlecitygis.opendata.arcgis.com/datasets/5b5c745e0f1f48e7a53acec63a0022ab_0.csv', 
...
#M[M>0]
as.data.frame(M[M>0]) %>% rename_at(1, ~'n_nulls')
```
```{r}
df[is.na(df$SEVERITYCODE) | is.na(df$SDOT_COLCODE) | is.na(df$SDOT_COLDESC), c('SEVERITYCODE','SDOT_COLCODE','SDOT_COLDESC','COLLISIONTYPE')]
```

#### SEVERITYCODE, SDOT_COLCODE and SDOT_COLDESC have only 1 record with a NULL value. 
#### All records have values for INCDATE and INCDTTM.
...
print('INCDATE ...')
df$INCDATE[0]
min(as.Date(df$INCDATE))
max(as.Date(df$INCDATE))
print('INCDTTM ...')
head(df$INCDTTM, 50)
```
#### INCDATE is a factor with format 'YYYY-MM-DDT00:00:00.000Z'
#### INCDTTM is a factor with format of either 'M/DD/YY HH:MM' or 'M/DD/YY'
...
```{r}
df %>% group_by(fct_explicit_na(UNDERINFL)) %>% count_pct
df %>% group_by(fct_explicit_na(INATTENTIONIND)) %>% count_pct
df %>% group_by(fct_explicit_na(SPEEDING)) %>% count_pct
df %>% group_by(fct_explicit_na(HITPARKEDCAR)) %>% count_pct

```
#### UNDERINFL values include 0, 1, N, Y and NULL, > 95% 0, N or NULL.
#### INATTENTIONIND values are 'Y' or NULL, >86% NULL.
#### SPEEDING values are 'Y' or NULL, >95% NULL.
#### HITPARKEDCAR values are 'Y' or 'N', >95% NULL.


### What % of count columns are > 0?
```{r}
cols <- c('PERSONCOUNT','PEDCOUNT','PEDCYLCOUNT','VEHCOUNT','INJURIES','SERIOUSINJURIES','FATALITIES')
...
```
#### > 88% collisions have non-zero values for PERSONCOUNT and VEHCOUNT 
#### < 28% collisions have non-zero values INJURIES
#### < 5% of PEDCOUNT, PEDCYLCOUNT, SERIOUSINJURIES and FATALITIES are non-zero values


### Assessment Observation Compilation
...
* INCDATE is datatype 'factor', and includes only dates.
* INCDATE is a factor with format 'YYYY-MM-DDT00:00:00.000Z'
* INCDTTM is a factor with format of either 'M/DD/YY HH:MM' or 'M/DD/YY'
* Minimum INCDATE = '2003-10-06T00:00:00.000Z'
* SEVERITYCODE, SDOT_COLCODE and SDOT_COLDESC have only record that is non-null.  
* SDOT_COLCODE contains categorical data but is datatype Integer.#### UNDERINFL values include 0, 1, N, Y and NULL, > 95% 0, N or NULL.
* INATTENTIONIND values are 'Y' or NULL, >86% NULL.
* SPEEDING values are 'Y' or NULL, >95% NULL.
* HITPARKEDCAR values are 'Y' or 'N', >95% NULL.
...
df$Month <- as.factor(lubridate::month(as.Date(df$INCDATE)))
  ...
  # trim records not between 2004-2018 (our report range)
#max(df$INCDATE)
...
## Trim
  # trim where SEVERITYCODE = '' (should be only 1 or a few records)
  #subset(df, SEVERITYCODE == '')
df <- subset(df, SEVERITYCODE != '')
...
### Report data structure after cleaning
...
The dataset consists of 205,370 rows (collision reports) and 28 columns (features). 
...
df_samp <- sample_n(df[, c('SEVERITYCODE', 'SDOTtype','COLLISIONTYPE',
                           ...
                           'PERSONCOUNT', 'DUI','WEATHER','INATTENTION',
...
# Create functions  to return dataframe of count and % of 1 or 2 given factors.
create_summary <- function(data, col1, col2) {
  col1 <- enquo(col1)
  col2 <- enquo(col2)
  result_summary <- data %>%
...
# Create function to return dataframe of count and % of 2 given factors.
...
```{r echo=FALSE, warning=FALSE, message=FALSE, Inattention}

ggplot(data = create_summary(df, Year, INATTENTION), aes(x = Year, y = perc, fill = INATTENTION)) +
  scale_fill_brewer(palette="Dark2")  + 
  geom_bar(stat='identity')  +
  scale_y_continuous(breaks = seq(0, 100, 10) ) +
  scale_y_continuous(labels = percent_format() ) +
...
p2 <- ggplot(data = create_summary(df, INATTENTION, SEVERITYDESC), 
       aes(x = INATTENTION, y = perc, fill = factor(SEVERITYDESC, levels=rev(severity_level_order)))) +
  scale_fill_brewer(palette="Dark2")  + 
  geom_bar(stat='identity') +
  scale_y_continuous(labels = percent_format()) +
  labs(fill = 'SEVERITYDESC') 
...
ggplot(data = create_summary(df, DUI, INATTENTION), 
       aes(x = DUI, y = perc, fill = INATTENTION)) +
  scale_fill_brewer(palette="Dark2")  + 
  geom_bar(stat='identity') +
  scale_y_continuous(labels = percent_format()) +
