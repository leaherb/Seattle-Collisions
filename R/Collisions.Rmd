...
```
**Observation:** date and datetime columns do not have any NULLs.
...
```
**Observation:** No, two rows are affected.
```{r}
df[is.na(df$SEVERITYCODE),]
```
**Observation:** The SEVERITYCODE=NA could be marked '0' instead because INJURY, SERIOUSINJURIES and FATALITIES = 0
...
```{r}
# Helper function to return freq of given column.
...
#### For columns that counts-of-things, what % of values are > 0?
```{r}
...
* HITPARKEDCAR values are 'Y' or 'N', >95% 'N'
...
* Clean binary variables (Y or N, or NA)

* Update SEVERITYCODE to 0 where NULL
