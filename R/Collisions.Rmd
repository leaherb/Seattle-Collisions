...
```
*Note:* all date and datetime columns have no NULL values.

#### A few category columns have only 1 NULL, are they all from the same row?
```{r}
df[is.na(df$SEVERITYCODE) | is.na(df$SDOT_COLCODE) | is.na(df$SDOT_COLDESC), 
   c('SEVERITYCODE','SDOT_COLCODE','SDOT_COLDESC','COLLISIONTYPE')]
```
...
writeLines('INCDATE ...')
head(df$INCDATE, 20)
paste('Date range: ', min(as.Date(df$INCDATE)), ' -> ', max(as.Date(df$INCDATE)))
writeLines('INCDTTM ...')
head(df$INCDTTM, 20)
```

#### For columns that counts-of-things, what % of values are non-zero?
```{r}
# Function to find % of non-zero values in columns that count things.
