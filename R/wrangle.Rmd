...
# extract time 'word' from date/time, will be NA if only date is present
df$time <- word(df$INCDTTM, 2)
# extract just the HH:MM from time (without date and timezone)
df$time <- format(strptime(df$time, "%H:%M"), format="%H:%M")
# get hour from HH:MM time string
df$hour <- substr(df$time, 0, 2)

# TODO: DROP THIS NEXT SECTION
#df$time <- as.POSIXct((strptime(word(df$INCDTTM,2), "%H:%M")), format='%H:%M')
#df$time <- word(df$INCDTTM, 2, 3)
#df$time <- format(strptime(df$time, "%I:%M:%S %p"), format="%H:%M")
#head(df$time)
#head(word(df$time, sep = ':'))
#head(substr(df$time,0,2))
