...
* COLLISIONTYPE has most generalized category differentiation, with only 10 non-null values, we do not know how this data was created
...
1. all 3 categories are reconcilable and do not contradict
2. if COLLISIONTYPE is a generalized category derived from ST_COLDESC
3. if #2 is true, if any value compromises made when COLLISIONTYPE values were derived
...
It appears that COLLISIONTYPE may have been created as a simplified version of ST_COLDESC, but if that is so then the COLLISIONTYPE create function is wrong, or the State Collision Code Dictionary is wrong. The ST_COLCODE/ST_COLDESC combinations in the database does not match the "State Collision Code Directory" for code=5:
...
```
**Observations:**
There is inconsistency between the 3 collision category columns when it comes to reporting Pedestrian and 'Pedcyclist' involvement. For example, some rows have:
...
#### Create SDOT_striker and SDOT_struck(ee) columns
```{r}
...
# State's interpretation of striker (the one doing)
df <- df %>%
    mutate(., ST_striker = with(., case_when(
...
df %>% 
    group_by(ST_striker, ST_struck, ST_COLDESC_collisiontype) %>%
    summarise(count = n())
...
df <- df %>%
    select(-OBJECTID, -REPORTNO, -STATUS, -EXCEPTRSNCODE, -EXCEPTRSNDESC,
           ...
           -PEDROWNOTGRNT, -LOCATION, -HITPARKEDCAR)
