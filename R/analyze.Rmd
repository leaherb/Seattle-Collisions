...
###\
3. What are the COLLISION types, according to the **SPD**?
...
ggplot(data = df,
    ...
    mytheme_minimal +
    mylayer_count_above +
    labs(title = 'SPD Collision Types',
         x = '', 
         y = '') +
    coord_flip()  
```
...
### \
4. What are the collision types as recorded by **SDOT**?
...
ggplot(data = df,
    ...
    mytheme_minimal +
    mylayer_count_above +
    labs(title = ' SDOT Collision Types',
         x = '', 
         y = '') +
    coord_flip()
```
...
\
**SPD**  and **SDOT** collision categories and their distributions are not the same. Which type I choose to include in this study could have an impact on our analysis. For example, if we are analyzing collisions involving Pedestrians, SPD reports only 1,770 'Pedestrian', while SDOT reports 'Hits Pedestrian' 17,846 times. That's a nearly 10x difference. Also, SDOT's 'Rear End' count is almost twice as many as SPD's.
...
The Severity factor alone does no quantify the damage done by each collision. 
...
# TODO: flip so labels are readable
