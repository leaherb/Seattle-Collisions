...
### 1. How have the total number of collisions changed over the years?
```{r}
...
**Observations: **
The distribution of collision frequency by year appears bimodal, peaking at **2005** and, to a lesser extent, **2015**, with a low in between at **2010** and again in **2018**. This plot hints at a possible 5-year pattern.

I had expected to see a steady increase in Seattle collisions, mirroring population growth. Perhaps I'm wrong about the steady increase in population, or that the number of **reported** collisions have a strong correlation with the population in the first place. #TODO: import population data and compare rate changes with collision rates by year.

###\
2. Do collision frequencies change throughout the day?
```{r warning=FALSE}
ggplot(data = df %>% 
           filter(!is.na(hour)) %>%
           count(hour), 
...
**Observations: **
It appears that collision counts tend to rise during commute hours, starting at the lowest point around 4am, reaching a peak at 5pm. 

TODO: Chart by hour by day of week (or weekday vs weekend)

###\
3. What are the COLLISION types?

> Our dataset includes 2 Collision type (category) variables: 
>
> 1.  COLLISIONTYPE (a derivation of the State's collision category)
>
> 2.  SDOT_striking recorded by SDOT (Seattle Department of Transportation)


```{r warning=FALSE, message=FALSE, Univariate_Plots}
# TODO: print % instead of totals, no x-axis labels / ticks
ggplot(data = df,
       aes(x = reorder(COLLISIONTYPE, COLLISIONTYPE, function(x) length(x) * 1)  )) +
    geom_bar(width = .85)   +
    mytheme_minimal +
    mylayer_count_above +
    labs(title = 'State Collision Types',
         x = '', 
         y = '') +
    coord_flip()  
```
**Observations: ***
The State collision types seem to be a mix of what is hit in the collision and how they're hit. 

### \
4. What is striking what?

```{r  warning=FALSE, message=FALSE}
# TODO: print % instead of totals, no x-axis / ticks
df$SDOT_strikes <- paste(df$SDOT_striker, ' STRUCK ', df$SDOT_struck)

df <- df %>%
    mutate(., SDOT_striking = with(., case_when(
        (SDOT_striker == 'Unknown / NA' & SDOT_struck == 'Unknown / NA') ~ 'Unknown',
        (SDOT_struck == 'Non Collision') ~ paste(SDOT_striker, '- Non Collision'),
        TRUE ~ paste(SDOT_striker, ' STRUCK ', SDOT_struck)
    )))

ggplot(data = df,
       aes(x = reorder(SDOT_striking, SDOT_striking, function(x) length(x) * 1))) +
    geom_bar()   +
    mytheme_minimal +
    geom_text(stat='count', aes(label = ..count..), vjust = .45) +
    labs(title = ' SDOT Strikings',
         x = '', 
         y = '') +
    coord_flip()

ggplot(data = df,
       aes(x = reorder(SDOT_struck, SDOT_struck, function(x) length(x) * 1))) +
    geom_bar()   +
    mytheme_minimal +
    geom_text(stat='count', aes(label = ..count..), vjust = .45) +
    labs(title = ' SDOT What is getting hit',
         x = '', 
         y = '') +
    coord_flip()
```
**Observations: **
It makes sense that most vehicle collisions involve a vehicle getting hit. 

More than twice as many pedestrians get hit than pedacyclists. 


### \
5. What are the SEVERITY categories?

```{r warning=FALSE, message=FALSE}
# TODO: remove decimal in %, add title
...
The Severity factor alone does no quantify the amount of damage done by each collision. 


### \
6. Geographical, environmental and time factors
...
# TODO: flip so labels are readable; sep add/junction type, weather/light/road, and season/day_part
...
* time factor (**season**, **day_part**) frequencies a more evenly spread, with the highest frequency in Summer Afternoons.


### \
7. The **Counts of Things**
...
#### \
8. Since many of the **Counts of Things** have 0 value, plot them again but this time with log10 so non-zero values are more visible.
...
* season
* hour
* day_part (morning, afternoon, etc)

I created a new collision categorical variable (SDOT_striking) to group common SDOT's collision types. For example, I combined 6 types of 'Sideswipe' collision types into one category called 'Sideswipe'.
...
set.seed(1965)
df_samp <- sample_n(df[, c('SEVERITYCODE', 
                           'SDOT_striking','COLLISIONTYPE',
                           'ADDRTYPE', 'JUNCTIONTYPE', 'VEHCOUNT', 
                           'PERSONCOUNT', 'DUI','WEATHER','DISTRACTED',
                           'ROADCOND','LIGHTCOND','SPEEDING','day_part','hour')], 10000)
...
df$SDOT_striker <- as.factor(df$SDOT_striker)
df2 <- data.frame(lapply(df[, 
                            c('DUI','VEHCOUNT','SEVERITYCODE','COLLISIONTYPE', 'SDOT_striker',
...
### \
Let's study of JUNCTIONTYPE vs ADDRTYPE.
...
p1 <- ggplot(data = create_summary(df, year, COLLISIONTYPE), 
             aes(x = year, y = perc, fill = factor(COLLISIONTYPE))) +
    geom_bar(stat='identity') +
    labs(fill = 'SDOT_striking')  +
    scale_y_continuous(labels = percent_format()) +
    scale_fill_brewer(palette = "Dark2")  
...
>There are several variables that indicate a Pedestrian was involved in a collision (COLLISIONTYPE, SDOT_type_striking, PEDCOUNT). Are all pedestrian-related collisions being marked by SDOT as 'Hits Pedestrian'? 
>
...
df$SDOT_striking.Adjusted <- df$SDOT_striking

df <- df %>%
    mutate(SDOT_striking.Adjusted = case_when(
        .$SDOT_struck == 'Pedestrian' ~ 'Hits Pedestrian',
        .$COLLISIONTYPE == 'Pedestrian' ~ 'Hits Pedestrian',
        .$PEDCOUNT >= 1 ~'Hits Pedestrian',
        TRUE ~ as.character(SDOT_striking)) )

ggplot(data = create_summary(subset(df, SDOT_striking.Adjusted == 'Hits Pedestrian'), SDOT_striking.Adjusted, SDOT_striking),
       aes(x = SDOT_striking.Adjusted, y = perc, fill = factor(SDOT_striking))) +
    scale_fill_brewer(palette = "Dark2")  + 
    geom_bar(stat='identity') +
    scale_y_continuous(labels = percent_format()) +
    labs(fill = 'SDOT_striking')  
...
p2 <- ggplot(data = create_summary(df, year, SDOT_striking.Adjusted), 
             aes(x = year, y = perc, fill = factor(SDOT_striking.Adjusted))) +
    geom_bar(stat='identity') +
    labs(fill = 'SDOT_striking.Adjusted') +
    scale_y_continuous(labels = percent_format()) +
    scale_fill_brewer(palette = "Dark2") 
...
p3 <- plot_under(subset(dfu, SDOT_striking != ''), DUI, SDOT_striking, aes(x = DUI, y = perc, fill = factor(SDOT_striking)))+
...
### \
Let's look a little closer at the SDOT_striking and LIGHTCOND plots in larger scale.


#### DUI and SDOT_striking
```{r echo=FALSE, warning=FALSE, message=FALSE}

plot_under(dfu, DUI, SDOT_striking, aes(x = DUI, y = perc, fill = factor(SDOT_striking)))
...
ggplot(data = time_perc,
       aes(x = hour, y = perc, color = LIGHTCOND, group = 1)) +
    geom_line(size = 2) +
    mylayer_color +
...
# TODO: filter out NA hours
ggplot(data = subset(by_year_hour, year %in% c(2005, 2010, 2015)),
       aes(x = hour, y = count)) +
    geom_line(aes(color = year), size = 1, group = 1) +
    ylim(0,1200) 
...
* I'm surprised that LIGHTCOND did not have a higher score with hour in the correlation tables, and had a weak relationship with day_part. 
...
ggplot(data = subset(dfu, !is.na(day_part) & SEVERITYDESC %in% c('Injury','Serious Injury','Fatality')), 
       aes(x = factor(day_part, level = c('Night','Evening','Afternoon','Morning')), 
           y = DUI) 
) +
...
# TODO: filter out NA hours, fix DUI line
# Now combine 2 plots
ggplot() +
    p_bar +
    p_line +
...
fatality_counts <- 
    ...
    group_by(year, SDOT_striking.Adjusted) %>%
    summarise(total = sum(FATALITIES))

p_f <- 
    geom_boxplot(data = fatality_counts,
                 aes(x = '', y = total,
                     color = SDOT_striking.Adjusted))

ggplot() +  p_f +
    facet_wrap(~ SDOT_striking.Adjusted)  +
    theme(legend.position = 'bottom', legend.direction = 'horizontal') +
    labs(title = 'Annual Fatality Counts by SDOT Collision Type', x = '', color = 'SDOT_striking.Adjusted') +
    mylayer_color 
```
...
fatality_counts2 <- 
    subset(df, SEVERITYDESC %in% c('Fatality') & SDOT_striking.Adjusted %in% c('Other Collision')) %>%
...
ggplot() +  p_f2 +
    ...
    theme(legend.position = 'bottom', legend.direction = 'horizontal') +
    labs(title = 'Annual Fatality Counts by SPD Collision Type WHEN SDOT is "Other"', x = '', color = 'SDOT_striking.Adjusted') +
    mylayer_color 
```
...
p2 <- ggplot(data = create_summary(df, VEHCOUNT, SDOT_striking),
             aes( fill = factor(SDOT_striking))) +
    geom_bar(aes(x = VEHCOUNT, y = perc), stat='identity') +
    scale_fill_brewer(palette="Paired")  +
...
# By time plot
g1 <- geom_point(data = df %>% count(hour), aes(x = hour, y = n, group = 1 ))
g2 <- geom_smooth(data = df %>% count(hour), aes(x = hour, y = n, group = 1 ), color='grey40')
g3 <- geom_line(data = df %>% count(hour), aes(x = hour, y = n, group = 1 )) 
...
ggplot(data = create_summary(df, SDOT_striking, COLLISIONTYPE), 
       aes(x = SDOT_striking, y = perc, fill = factor(COLLISIONTYPE))) +
    scale_fill_brewer(palette="Paired")  + 
    geom_bar(stat='identity') +
    scale_y_continuous(labels = percent_format()) +
